    ...
    -- Safely add constraints to prevent duplicate usernames and email_ids only if users table exists
    DO $$
    BEGIN
        IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'users') THEN
            ...
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'unique_email_id') THEN
