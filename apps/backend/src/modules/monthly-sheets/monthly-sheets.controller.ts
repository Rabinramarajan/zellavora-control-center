...
export class MonthlySheetsController {
  ...
  async update(@Req() req: AuthRequest, @Param('id') id: string, @Body() body: any) {
    const dto = UpdateMonthlySheetSchema.parse(body);
  ...
  async approve(@Req() req: AuthRequest, @Param('id') id: string, @Body() body: any) {
    const dto = ApproveMonthlySheetSchema.parse(body);
    return this.service.approve(id, dto, req.organizationId, req.userId);
  }

  @Post(':id/mark-paid')
  async markAsPaid(@Req() req: AuthRequest, @Param('id') id: string, @Body() body: any) {
    const dto = MarkAsPaidSchema.parse(body);
    return this.service.markAsPaid(id, dto, req.organizationId, req.userId);
  }
