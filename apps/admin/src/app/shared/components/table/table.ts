...
export class Table<T extends object> {
  ...
  protected cellTemplate = (k: string) =>
    this.cells().find((c) => c.appCell() === k)?.template ?? null;
  ...
  constructor() {
    ...
    effect(() => {
      if (this.reorderable() && this.virtualScroll())
        console.warn(
          '[data-table] reorderable and virtualScroll are mutually exclusive; disable one.'
        );
    });
  }
  ...
  protected readonly sorted = computed<readonly T[]>(() => {
    ...
    return [...this.filtered()].sort((a, b) => {
      const av = this.rawValue(col, a),
        bv = this.rawValue(col, b);
      if (av == null) return 1;
      if (bv == null) return -1;
  ...
  protected readonly total = computed(() =>
    this.serverSide() ? (this.totalCount() ?? this.source().length) : this.sorted().length
  );
  ...
  /* ---- values ---- */
  protected rawValue = (c: ColumnDef<T>, r: T) =>
    c.value ? c.value(r) : (r as Record<string, unknown>)[c.key];
  protected display = (c: ColumnDef<T>, r: T) => {
    const v = this.rawValue(c, r);
    return c.format ? c.format(v, r) : String(v ?? '');
  };
  protected trackRow = (i: number, r: T) => (this.trackBy() ? this.trackBy()!(r) : i);
  protected skeletonArray = computed(() =>
  ...
  protected sortIndicator = (c: ColumnDef<T>) =>
    this.sort().key === c.key ? this.sort().dir : null;
  protected ariaSort = (c: ColumnDef<T>) => {
    const d = this.sortIndicator(c);
  ...
  protected allSelected = computed(
    () => this.pageRows().length > 0 && this.pageRows().every((r) => this.isSelected(r))
  );
  ...
  protected isEditing = (r: T, c: ColumnDef<T>) =>
    this.editingRow() === r && this.editingKey() === c.key;
  protected beginEdit(r: T, c: ColumnDef<T>) {
    this.editingRow.set(r);
    this.editingKey.set(c.key);
  }
  ...
  protected menuSort(key: string, dir: SortDir) {
  ...
  protected async copyColumn(key: string) {
    const col = this.columns().find((c) => c.key === key);
    if (!col) return;
    const text = this.sorted()
      .map((r) => this.display(col, r))
      .join('\n');
    try {
